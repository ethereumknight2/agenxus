        ...
        <section className="mb-20">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-8 mb-16">
            {solutions.map((solution, index) => {
              // Map solution names to their appropriate icons
              const getIcon = (solutionName: string) => {
                if (solutionName.includes("Voice")) return Phone;
                if (solutionName.includes("Chat")) return MessageCircle;
                if (solutionName.includes("Video")) return Video;
                if (
                  solutionName.includes("Search") ||
                  solutionName.includes("AEO")
                )
                  return Search;
                return Cog; // Default for automation/other solutions
              };

              // Map solution names to their appropriate gradients
              const getGradient = (solutionName: string) => {
                if (solutionName.includes("Voice"))
                  return "from-blue-500 to-cyan-500";
                if (solutionName.includes("Chat"))
                  return "from-purple-500 to-pink-500";
                if (solutionName.includes("Video"))
                ...
                return "from-orange-500 to-red-500"; // Default for automation/other solutions
              };

              const IconComponent = getIcon(solution.name);
              const gradient = getGradient(solution.name);
              ...
              return (
                ...
                >
                  <div
                    className={`w-16 h-16 bg-gradient-to-r ${gradient} rounded-full flex items-center justify-center mx-auto mb-4 group-hover:scale-110 transition-transform`}
                  >
