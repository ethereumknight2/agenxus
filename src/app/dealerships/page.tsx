...
// Note: Remove the export metadata since we're using "use client"
// Instead, we'll add metadata in a separate layout or use dynamic metadata
...
interface ROIInputs {
  monthlyServiceAppointments: number;
  averageServiceRevenue: number;
  currentNoShowRate: number;
  monthlyCarsOld: number;
  averageCarProfit: number;
...
const ROICalculator = () => {
  ...
  const [results, setResults] = useState<Partial<ROIResults>>({});
  ...
  return (
    <div className="bg-gradient-to-br from-purple-900/20 to-blue-900/20 backdrop-blur-sm rounded-2xl p-8 border border-purple-500/30">
      <div className="text-center mb-8">
        ...
        <p className="text-gray-300">See your potential return on investment</p>
      </div>
          ...
          <div className="grid md:grid-cols-3 gap-4">
            <div className="bg-gradient-to-r from-purple-600/20 to-pink-600/20 rounded-xl p-6 border border-purple-400/30">
              <div className="text-center">
                ...
                <div className="text-2xl font-bold text-white">
                  {formatCurrency(results.monthlyServiceBenefit || 0)}
                </div>
                <div className="text-sm text-purple-300">
                  Monthly Service Boost
                </div>
                <div className="text-xs text-gray-400 mt-1">
                  {Math.round(results.recoveredAppointments || 0)} recovered
                  appointments
                </div>
              </div>
            </div>
            ...
            <div className="bg-gradient-to-r from-blue-600/20 to-cyan-600/20 rounded-xl p-6 border border-blue-400/30">
              <div className="text-center">
                ...
                <div className="text-2xl font-bold text-white">
                  {formatCurrency(results.monthlySalesBenefit || 0)}
                </div>
                <div className="text-sm text-blue-300">Monthly Sales Boost</div>
                <div className="text-xs text-gray-400 mt-1">
                  {Math.round(results.additionalCarsSold || 0)} additional cars
                  sold
                </div>
              </div>
            </div>
            ...
            <div className="bg-gradient-to-r from-green-600/20 to-emerald-600/20 rounded-xl p-6 border border-green-400/30">
              <div className="text-center">
                ...
                <div className="text-2xl font-bold text-white">
                  {formatCurrency(results.netMonthlyBenefit || 0)}
                </div>
                <div className="text-sm text-green-300">Net Monthly Profit</div>
                <div className="text-xs text-gray-400 mt-1">
                  After all costs
                </div>
              </div>
                ...
                >
                  ...
                  <div
                    className={`text-sm font-medium ${
                      result.netProfit >= 0 ? "text-green-300" : "text-red-300"
                    }`}
                  >
...
export default function DealershipsPage() {
  useEffect(() => {
    // Replace gtag with Next.js compatible analytics if needed
  ...
  useEffect(() => {
    // Which element should we scroll to?
    const getTargetId = () => {
    ...
    // How far to offset (sticky header + CSS scroll-margin-top)
    const headerOffsetPx = () => {
      ...
      const smt = roi
        ? parseInt(getComputedStyle(roi).scrollMarginTop || "0", 10) || 0
        : 0;

      // Use whichever is larger; fallback 96px
      return Math.max(stickyH, smt, 96);
    };
    ...
    // Try to find & scroll to the target; retry while the DOM hydrates/mounts
    let tries = 0;
    const maxTries = 40; // up to ~8s total
    ...
    const tryScroll = () => {
      ...
      if (el) {
        // Double RAF to ensure layout is painted before measuring
        requestAnimationFrame(() =>
          requestAnimationFrame(() => scrollToEl(el))
        );
        return;
      }
      if (tries++ < maxTries) setTimeout(tryScroll, interval);
    };

    // Run on load, on hash changes, and after window 'load'
    tryScroll();
    const onHash = () => tryScroll();
    window.addEventListener("hashchange", onHash);
    window.addEventListener("load", tryScroll);

    // Also watch the DOM: if the ROI block mounts late, we still catch it
  ...
  const handleAnalyticsEvent = (eventName: string, location: string) => {
    if (typeof window !== "undefined" && window.gtag) {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-900 via-blue-950 to-black text-white overflow-x-hidden">
          ...
          <main>
            ...
            <div className="mb-8 text-center">
              ...
              <a
                ...
                onClick={() => handleAnalyticsEvent("click_call_ai", "hero")}
              >
            ...
            <div className="flex flex-col md:flex-row gap-4 justify-center items-center mb-8">
              <a
                ...
                onClick={() => handleAnalyticsEvent("click_book_demo", "hero")}
              >
      ...
      <section
        id="roi-calculator"
        className="py-20 px-6 scroll-mt-24 md:scroll-mt-32"
        style={{ scrollMarginTop: "96px" }} // fallback if classes don’t apply
      >
        <div className="container mx-auto">
          <div className="text-center mb-16">
            <h2 className="text-4xl md:text-5xl font-bold mb-6">
            ...
            <p className="text-xl text-gray-300 max-w-3xl mx-auto">
              ...
              solution. Adjust the inputs to match your dealership&apos;s
              current performance.
            </p>
          </div>
          ...
          <div className="text-center mt-12">
            <div className="flex flex-col sm:flex-row gap-4 justify-center">
              <a
                ...
                onClick={() =>
                  handleAnalyticsEvent("click_call_ai", "roi_section")
                }
              >
              ...
              <a
                ...
                onClick={() =>
                  handleAnalyticsEvent("click_book_demo", "roi_section")
                }
              >
                <Calendar className="w-5 h-5" />
        ...
        <div className="container mx-auto text-center">
          ...
          <div className="flex justify-center">
            <a
              ...
              onClick={() => handleAnalyticsEvent("click_book_demo", "footer")}
            >
      ...
      <div className="md:hidden fixed bottom-0 inset-x-0 z-50 bg-black/90 backdrop-blur border-t border-white/10">
        <div className="p-3 flex gap-3">
          <a
            ...
            onClick={() =>
              handleAnalyticsEvent("click_call_ai", "sticky_mobile")
            }
          >
          ...
          <a
            ...
            onClick={() =>
              handleAnalyticsEvent("click_book_demo", "sticky_mobile")
            }
          >
